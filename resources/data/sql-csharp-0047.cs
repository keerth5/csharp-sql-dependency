...
namespace SampleApp
{
    public class NolockHintUsageExample
    {
        ...
        // ❌ Violation: Hard-coded NOLOCK hint in SQL
        public void Bad_HardCodedNolock()
        {
            string sql = "SELECT Name, Role FROM Users WITH (NOLOCK) WHERE IsActive = 1";
        ...
        // ❌ Violation: Dynamic NOLOCK hint with string concatenation
        public void Bad_DynamicNolock(string tableName)
        {
            string sql = $"SELECT Name, Role FROM {tableName} WITH (NOLOCK) WHERE IsActive = 1";
        ...
        // ✅ Compliant: Parameterized query without NOLOCK hint
        public async Task Good_ParameterizedQuery(string tableName)
        {
            string sql = $"SELECT Name, Role FROM {tableName} WHERE IsActive = @IsActive";
        ...
        // ✅ Compliant: EF Core abstraction avoids NOLOCK hint
        public async Task Good_EntityFrameworkQuery(MyDbContext dbContext)
        {
