...
namespace SampleApp
{
    public class CrossApplyUsageExample
    {
        private readonly string _connectionString = "Server=.;Database=TestDb;Trusted_Connection=True;";

        // ❌ Violation: Using CROSS APPLY in SQL Server query
        public void Bad_CrossApplyQuery()
        {
            string sql = @"
        ...
        // ❌ Violation: Dynamic CROSS APPLY SQL
        public void Bad_DynamicCrossApply(string topCountColumn)
        {
            string sql = $@"
                SELECT e.EmployeeId, e.Name, s.SalesAmount
                FROM Employees e
        ...
        // ✅ Compliant: Application-side join without CROSS APPLY
        public async Task Good_ApplicationJoin(MyDbContext dbContext)
        {
        ...
        // ✅ Compliant: EF Core LINQ join instead of CROSS APPLY
        public async Task Good_EFCoreJoin(MyDbContext dbContext)
        {
